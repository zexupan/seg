...
class Solver(object):
    ...
    def _reset(self):
        ...
        if self.args.continue_from:
            ...
            checkpoint = torch.load('/home/panzexu/workspace/avss_pose/log/sync/avConv_2022-01-16(20:31:04)/model_dict.pt', map_location='cpu')
            self.model.load_state_dict(checkpoint['model'])
            self.optimizer.load_state_dict(checkpoint['optimizer'])
            self.amp.load_state_dict(checkpoint['amp'])
            print("loaded from avConv_2022-01-16(20:31:04)")
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.args.epochs+1):
            ...
            start = time.time()
            with torch.no_grad():
                test_loss, test_acc = self._run_one_epoch(data_loader = self.test_data, state='test')
                reduced_test_loss = self._reduce_tensor(test_loss)
                reduced_test_acc = self._reduce_tensor(test_acc)
            ...
            if self.print:
                ...
                if self.args.use_tensorboard:
                    ...
                    self.writer.add_scalar('Test_acc', reduced_test_acc, epoch)
    ...
    def _run_one_epoch(self, data_loader, state='train'):
        ...
        for i, (label, audio, visual) in enumerate(data_loader):
