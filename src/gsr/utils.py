...
class dataset(data.Dataset):
    def __init__(self,
        ...
        mix_lst = sorted(mix_lst, key=lambda data: float(data.split(',')[-2]), reverse=True)
    ...
    def __getitem__(self, index):
        ...
        min_length = math.floor(float(batch_lst[-1].split(',')[-2])*self.sampling_rate)
        ...
        return np.asarray(labels), \
                ...
                np.asarray(visuals)[...,:self.max_length*15,:]
